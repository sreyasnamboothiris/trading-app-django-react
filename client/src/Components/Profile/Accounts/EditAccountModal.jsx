...
function EditAccountModal({ account,onUpdate  }) {
  const [showModal, setShowModal] = useState(false);
  const [accountName, setAccountName] = useState(account.name || '');
  const [currency, setCurrency] = useState(account.currency || '');
  const [funds, setFunds] = useState(account.funds || 0);
  ...
  const handleEditAccount = () => {
    ...
    api
      ...
      .catch((error) => {
        const errorMessage = error.response?.data?.message || 'Error updating account';
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-[#2D5F8B] border-black border-4 rounded-xl w-96">
            <div className="bg-[#002F42] w-full p-2 flex justify-center">
              <h2 className="text-4xl font-semibold mb-4">Edit Account</h2>
            </div>
